...
def manual_add_attendance():
    ...
    if _is_ajax():
        progress       = get_student_progress(teacher.id)
        unbilled_count = Attendance.query.filter_by(teacher_id=teacher.id, billed=False).count()
        ...
        return jsonify(
            ok=True, msg=tr("ok_attn"), record_html=record_html,
            progress_html=progress_html, unbilled_count=unbilled_count,
            student_id=student_id, s_total=s_total, s_unbilled=s_unbilled,
    ...
    if delete_attendance(att_id, teacher.id):
        if _is_ajax():
            ...
            return jsonify(ok=True, msg=tr("ok_deleted"),
                           progress_html=progress_html, student_id=student_id,
...
def edit_receipt_time(receipt_id):
    ...
    try:
        ...
        if _is_ajax(): return jsonify(ok=False, msg="Invalid date format"), 400
    return redirect(url_for("teacher.dashboard"))
